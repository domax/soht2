...
class ConnectionControllerTest {
  ...
  @MockitoBean Soht2Service soht2Service;
  ...
  void open_OK() throws Exception {
    ...
    try (val localDateTime = mockStatic(LocalDateTime.class);
      ...
      doReturn(connectionInfo).when(soht2Service).open(any(Soht2Connection.class));

      mockMvc
          .perform(
              post("/api/connection").queryParam("host", "targetHost").queryParam("port", "12345"))
          .andExpect(status().isOk())
          .andExpect(content().contentType(APPLICATION_JSON))
          .andExpect(jsonPath("$.id").value(sohtConnection.id().toString()))
          .andExpect(jsonPath("$.username").value(sohtConnection.username()))
      ...
      verify(soht2Service).open(sohtConnection);
